...
export default function RecipeCard({ recipe }) {
  ...
  const { isHighlighted, toggleHighlight } = useCalendar()
  const highlighted = isHighlighted(recipe.id)
  ...
  return (
    <Link to={`/recipe/${recipe.id}`} className={styles.card}>
      <div className={styles.imageWrap}>
        ...
        <button
          className={`${styles.calendarBtn} ${highlighted ? styles.calendarBtnActive : ''}`}
          onClick={handleCalendarClick}
          aria-label={highlighted ? 'Remove from calendar pool' : 'Add to calendar pool'}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
